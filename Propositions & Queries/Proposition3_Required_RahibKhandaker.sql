...
select CourseCode, count(a.CourseId) As ClassCountinCourse, sum(Enrollment) as TotalEnrollment, sum(Limit) as TotalLimit, (sum(Enrollment) * 100/ sum(Limit)) as EnrollPercent from Course.CoursesCLassMode a
